...
namespace SushiWebsite.Controllers
{
    public class MenuController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> PlaceOrder(string firstName, string lastName, string phoneNumber, DateTime diningDateTime)
        {
            // Kiểm tra thông tin đầu vào
            if (!IsValidOrder(firstName, lastName, phoneNumber, diningDateTime, out string validationError))
            {
            ...
            string message = CreateOrderMessage(firstName, lastName, phoneNumber, diningDateTime, Cart);
        ...
        private bool IsValidOrder(string firstName, string lastName, string phoneNumber, DateTime diningDateTime, out string errorMessage)
        {
            if (string.IsNullOrWhiteSpace(firstName) ||
                string.IsNullOrWhiteSpace(lastName) ||
                string.IsNullOrWhiteSpace(phoneNumber) ||
                diningDateTime == default)
            {
        ...
        private string CreateOrderMessage(string firstName, string lastName, string phoneNumber, DateTime diningDateTime, List<CartItem> cartItems)
        {
...
- Dining Date and Time: {diningDateTime:dd/MM/yyyy HH:mm}
...
Total Amount: {cartItems.Sum(item => item.Quantity * item.Price):C}
        ...
        {
            string telegramBotToken = "7909330094:AAHro7wVhkTS6r1gH4bYWluYWNjy84raNbA";
            string chatId = "6319807639";
